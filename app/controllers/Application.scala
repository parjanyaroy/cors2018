...
class Application @Inject() (wsClient: WSClient, cache: CacheApi) (implicit ec: ExecutionContext) extends Controller {
  ...
  object CorsAction extends ActionBuilder[Request] {
    override def invokeBlock[A](request: Request[A], block: (Request[A]) => Future[Result]): Future[Result] = {
      block(request).map(result => result.withHeaders(HeaderNames.ACCESS_CONTROL_ALLOW_ORIGIN -> "*",HeaderNames.ACCESS_CONTROL_ALLOW_METHODS -> "POST,GET,OPTIONS,PATCH"))
    }
  }
  ...
  def options(path: String) = CorsAction {
    Ok.withHeaders(ACCESS_CONTROL_ALLOW_HEADERS -> Seq(AUTHORIZATION, CONTENT_TYPE, "Target-URL","x-prettyprint").mkString(","))
  }
